    ...
    <div class="right">
      <el-row :gutter="20">
        ...
        <el-col :span="7" style="padding: 0">
          <el-input class="humidity-input" type="number" v-model="inputModel" size="mini"
                    :placeholder="'当前阈值：' + option.series[0].markLine.data[0].yAxis + '%RH'">
          </el-input>
      ...
      <Row class="right-table">
        <el-table border size="small" :data="rightData" height="calc(100vh - 185px)">
          ...
          <el-table-column align="center" label="湿度值" width="125" prop="value">
...
<script>
  ...
  import {humidityService, typeService,exportService} from '../../api/service'
  ...
  export default {
    ...
    data() {
      return {
        ...
        inputModel: null,
        option: {
          title: {text: '坝体湿度变化情况', x: 'center'},
          tooltip: {},
          toolbox: websocketUtil.toolbox,
          grid: websocketUtil.grid,
          xAxis: {type: 'category', data: [], axisLabel: {interval: 0, rotate: 40}},
          yAxis: {type: 'value', axisLabel: {formatter: '{value} ( %RH)'}},
          series: [{name: '湿度', type: 'line', data: [], markLine: {data: [{name: '阈值线', yAxis: 0,}]}}],
    ...
    computed: mapState(['humidityRecvData']),
    watch: {
      humidityRecvData(newVal){
    ...
    methods: {
      ...
      findByBigTypeAndSubIndex() {
        return typeService.findByBigTypeAndSubIndex({bigType: this.bigType,subIndex: 0}).then(res => res.code === 0 ? this.setLimitData(res.obj.limitValue) : null)
      },
      updateLimitValue() {
